...
public class CustomViewAbove extends ViewGroup {
		...
		if (isMenuOpen()) {
			...
			case SlidingMenu.TOUCHMODE_MARGIN:
				if (isInIgnoredView(ev)) {
					// TODO : Potential risk. if it is a whole screen view, touch margin and swipe won't work. 
					return false;
				}
	...
	boolean mBehindEventCanceled = false;
	@Override
	public boolean onTouchEvent(MotionEvent ev) {
		...
		//解决fullscreen模式下behind view无法响应事件的问题
		if (mIsBeingDragged) {
			if (!mBehindEventCanceled) {
				MotionEvent cancelEvent = MotionEvent.obtain(ev);
				cancelEvent.setAction(MotionEvent.ACTION_CANCEL);
				mViewBehind.dispatchTouchEvent(cancelEvent);
				cancelEvent.recycle();
				mBehindEventCanceled = true;
			}
		} else {
