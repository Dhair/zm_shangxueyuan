...
import com.sdk.download.providers.downloads.DownloadHandler;
import com.sdk.download.providers.downloads.Downloads;
import com.sdk.download.providers.downloads.ui.DownloadListActivity;
...
public class DownloadManager {
	...
	public static final String COLUMN_OTHER_KEY = "other_key";
	public static final String COLUMN_IS_NEED_POST_DATA = "is_need_post_data";
	public static final String COLUMN_APK_PACKAGE_NAME = "apk_package_name";
	...
	private static final String[] COLUMNS = new String[] { COLUMN_ID, COLUMN_TITLE, COLUMN_DESCRIPTION, COLUMN_URI, COLUMN_MEDIA_TYPE, COLUMN_TOTAL_SIZE_BYTES,
			COLUMN_LOCAL_URI, COLUMN_STATUS, COLUMN_REASON, COLUMN_BYTES_DOWNLOADED_SO_FAR, COLUMN_LAST_MODIFIED_TIMESTAMP, COLUMN_PRIMARY_KEY,
			COLUMN_IS_NEED_POST_DATA, COLUMN_APK_PACKAGE_NAME, COLUMN_OTHER_KEY, COLUMN_DOWNLOAD_URL };
	...
	private static final String[] UNDERLYING_COLUMNS = new String[] { Downloads._ID, Downloads.COLUMN_TITLE, Downloads.COLUMN_DESCRIPTION,
			...
			Downloads.COLUMN_LAST_MODIFICATION, Downloads.COLUMN_DESTINATION, Downloads.COLUMN_FILE_NAME_HINT, Downloads._DATA, Downloads.COLUMN_PRIMARY_KEY,
			Downloads.COLUMN_IS_NEED_POST_DATA, Downloads.COLUMN_APK_PACKAGE_NAME, Downloads.COLUMN_OTHER_KEY, Downloads.COLUMN_DOWNLOAD_URL };

	private static final Set<String> LONG_COLUMNS = new HashSet<String>(Arrays.asList(COLUMN_ID, COLUMN_TOTAL_SIZE_BYTES, COLUMN_STATUS, COLUMN_REASON,
	...
	public static class Request {
		...
		private CharSequence mNotiExtra;

		private Uri mUri;
		private String mPrimaryKey = "";
		private String mOtherKey = "";
		private String mIsNeedPostData = "false";
		private Uri mDestinationUri;
		private List<Pair<String, String>> mRequestHeaders = new ArrayList<Pair<String, String>>();
		private CharSequence mTitle;
		private CharSequence mCountryName;
		...
		public Request setNotiClass(CharSequence notiClass) {
			this.mNotiClass = notiClass;
			return this;
		}

		public Request setNotiExtra(CharSequence notiExtra) {
			this.mNotiExtra = notiExtra;
			return this;
		}
		 ...
		 * {@link com.zuimeia_sdk_download.providers.NotificationManager}. By
		...
		public Request setCountryName(CharSequence countryName) {
		...
		ContentValues toContentValues(String packageName) {
			...
			values.put(Downloads.COLUMN_PRIMARY_KEY, mPrimaryKey);
			values.put(Downloads.COLUMN_DOWNLOAD_URL, mUri.toString());
			values.put(Downloads.COLUMN_OTHER_KEY, mOtherKey);
			values.put(Downloads.COLUMN_APK_PACKAGE_NAME, "");
			values.put(Downloads.COLUMN_IS_NEED_POST_DATA, mIsNeedPostData);
			...
			// wyx add
			putIfNonNull(values, Downloads.COLUMN_NOTIFICATION_CLASS, mNotiClass);
			putIfNonNull(values, Downloads.COLUMN_NOTIFICATION_EXTRAS, mNotiExtra);
			putIfNonNull(values, Downloads.COLUMN_COUNTRY_NAME, mCountryName);
			// wyx add
	 ...
	 * @hide
	 */
	public DownloadManager(Context context, ContentResolver resolver, String packageName) {
		mResolver = resolver;
		mPackageName = packageName;
		mContext = context;
		mBaseUri = Downloads.getContentURI(context);
	}
	...
	public void setAccessAllDownloads(boolean accessAllDownloads) {
		if (accessAllDownloads) {
			mBaseUri = Downloads.getAllDownloadsContentURI(mContext);
		} else {
			mBaseUri = Downloads.getContentURI(mContext);
		}
	}
	...
	public long enqueue(Request request) {
		try {
			...
			Uri downloadUri = mResolver.insert(Downloads.getContentURI(mContext), values);
			long id = Long.parseLong(downloadUri.getLastPathSegment());

			return id;
		} catch (Throwable t) {
			Log.e("", "downloadId =  " + t.getMessage());
			t.printStackTrace();
		}
		return -100;
	}

	public static void openDownload(Context mContext) {
		Intent it = new Intent(mContext, DownloadListActivity.class);
	...
	public int remove(long... ids) {
		...
		for (long id : ids) {
			DownloadHandler.getInstance().dequeueWaitDownload(id);
		}
	...
	private static class CursorTranslator extends CursorWrapper {
		...
		private String translateString(String column) {
			...
			if (column.equals(COLUMN_PRIMARY_KEY)) {
				return getUnderlyingString(Downloads.COLUMN_PRIMARY_KEY);
			}
			if (column.equals(COLUMN_DOWNLOAD_URL)) {
				return getUnderlyingString(Downloads.COLUMN_DOWNLOAD_URL);
			}
			if (column.equals(COLUMN_OTHER_KEY)) {
				return getUnderlyingString(Downloads.COLUMN_OTHER_KEY);
			}
			if (column.equals(COLUMN_IS_NEED_POST_DATA)) {
				return getUnderlyingString(Downloads.COLUMN_IS_NEED_POST_DATA);
			}
			if (column.equals(COLUMN_APK_PACKAGE_NAME)) {
				return getUnderlyingString(Downloads.COLUMN_APK_PACKAGE_NAME);
			}
