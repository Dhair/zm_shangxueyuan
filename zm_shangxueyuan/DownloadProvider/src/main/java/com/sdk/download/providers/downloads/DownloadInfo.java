...
import com.sdk.download.utils.LogTimber;
...
public class DownloadInfo {
	...
	public static class Reader {
		...
		public void updateFromDatabase(DownloadInfo info) {
			...
			info.mPrimaryKey = getString(Downloads.COLUMN_PRIMARY_KEY);
			info.mDownloadUrl=getString(Downloads.COLUMN_DOWNLOAD_URL);
			info.mOtherKey = getString(Downloads.COLUMN_OTHER_KEY);
			info.apkPackageName = getString(Downloads.COLUMN_APK_PACKAGE_NAME);
			info.mIsNeedPostData = getString(Downloads.COLUMN_IS_NEED_POST_DATA);
			info.mNoIntegrity = getInt(Downloads.COLUMN_NO_INTEGRITY) == 1;
			info.mHint = getString(Downloads.COLUMN_FILE_NAME_HINT);
			info.mFileName = getString(Downloads._DATA);
			info.mMimeType = getString(Downloads.COLUMN_MIME_TYPE);
			...
			info.mPackage = getString(Downloads.COLUMN_NOTIFICATION_PACKAGE);
			info.mClass = getString(Downloads.COLUMN_NOTIFICATION_CLASS);
			info.mExtras = getString(Downloads.COLUMN_NOTIFICATION_EXTRAS);
			info.mCookies = getString(Downloads.COLUMN_COOKIE_DATA);
			info.mUserAgent = getString(Downloads.COLUMN_USER_AGENT);
			info.mReferer = getString(Downloads.COLUMN_REFERER);
			...
			info.mTitle = getString(Downloads.COLUMN_TITLE);
			info.mDescription = getString(Downloads.COLUMN_DESCRIPTION);
	...
	public void sendIntentIfRequested() {
		...
		LogTimber.d("sendIntentIfRequested >> mIsPublicApi:%s == mPackage:%s == mClass:%s", mIsPublicApi, mPackage, mClass);
		Intent intent;
		if (mIsPublicApi) {
	...
	private int checkIsNetworkTypeAllowed(int networkType) {
		if (mIsPublicApi) {
			...
			// if(mStatus == Downloads.STATUS_WAITING_FOR_NETWORK && flag !=
	...
	public Uri getMyDownloadsUri() {
		return ContentUris.withAppendedId(Downloads.getContentURI(mContext), mId);
	}

	public Uri getAllDownloadsUri() {
		return ContentUris.withAppendedId(Downloads.getAllDownloadsContentURI(mContext), mId);
	}
