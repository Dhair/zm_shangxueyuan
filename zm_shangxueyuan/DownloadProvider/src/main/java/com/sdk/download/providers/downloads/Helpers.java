...
public class Helpers {
    ...
    private static String getPathForFileUri(String url, String hint,
        ...
        if (path.endsWith("/")) {
            ...
            LogTimber.i("getPathForFileUri >> " + baseDir.toString());
            if (!baseDir.isDirectory() && !baseDir.mkdirs()) {
                ...
                throw new GenerateSaveFileError(Downloads.STATUS_FILE_ERROR,
                        "getPathForFileUri mathod unable to create external downloads directory "
                                + baseDir.getPath());
            }
    ...
    private static File getExternalDestination(long contentLength)
        ...
        if (!base.isDirectory() && !base.mkdirs()) {
            ...
            // read-only.
            throw new GenerateSaveFileError(Downloads.STATUS_FILE_ERROR,
                    "getExternalDestination mathod unable to create external downloads directory "
                            + base.getPath());
        }
        return base;
    }
    ...
    public static boolean isSDCardAvailable(Context context, long contentLength){
        boolean canwrite = false;
        if (Helpers.isExternalMediaMounted()) {
            File root = Environment.getExternalStorageDirectory();
            if (Helpers.getAvailableBytes(root) < contentLength) {
                canwrite = false;
            } else {
                canwrite = true;
            }
     ...
     */
    /* package */
    static void deleteFile(Context mContext,ContentResolver resolver, long id,
        ...
        resolver.delete(Downloads.getAllDownloadsContentURI(mContext), Downloads._ID
