...
/**
 * Receives system broadcasts (boot, network connectivity，pause,cancel)
 */
public class DownloadReceiver extends BroadcastReceiver {
	SystemFacade mSystemFacade = null;
	public static final String DOWNLOAD_PAUSE = "android.intent.action.DOWNLOAD_PAUSE";
	public static final String DOWNLOAD_CANCEL = "android.intent.action.DOWNLOAD_CANCEL";
	public static final String INTENT_DOWNLOAD_ID = "android.intent.DOWNLOAD_ID";
	public static final String INTENT_DOWNLOAD_STATUS = "android.intent.DOWNLOAD_STATUS";
	...
	public void onReceive(Context context, Intent intent) {
		...
		LogTimber.d("DownloadReceiver >>> " + intent.getAction());
		...
		} else if (action.equals(DOWNLOAD_PAUSE)) {// 暂停下载
			long mId = intent.getLongExtra(INTENT_DOWNLOAD_ID, -100);
			int mStatus = intent.getIntExtra(INTENT_DOWNLOAD_STATUS, -100);
			if (mId >= 0 && mStatus >= 0) {
				if (mStatus == Downloads.STATUS_PENDING || mStatus == Downloads.STATUS_RUNNING) {
					DownloadManager mDownloadManager = new DownloadManager(context, context.getContentResolver(), context.getPackageName());
					mDownloadManager.pauseDownload(mId);
				}
			}

		} else if (action.equals(DOWNLOAD_CANCEL)) {// 取消下载
			long mId = intent.getLongExtra(INTENT_DOWNLOAD_ID, -100);
			int mStatus = intent.getIntExtra(INTENT_DOWNLOAD_STATUS, -100);
			if (mId >= 0 && mStatus >= 0) {
				DownloadManager mDownloadManager = new DownloadManager(context, context.getContentResolver(), context.getPackageName());
				mDownloadManager.markRowDeleted(mId);
	 ...
	 *            Cursor for reading the download's fields
	 */
	private void sendNotificationClickedIntent(Context mContext, Intent intent, Cursor cursor) {
		LogTimber.d("sendNotificationClickedIntent");
		...
		if (isPublicApi) {
			...
			if (intent.getBooleanExtra("multiple", true)) {
				appIntent.setData(Downloads.getContentURI(mContext));
			} else {
				long downloadId = cursor.getLong(cursor.getColumnIndexOrThrow(Downloads._ID));
				appIntent.setData(ContentUris.withAppendedId(Downloads.getContentURI(mContext), downloadId));
			}
		}
